...
2. Find candidates economically: titles, aliases, headings, existing link neighborhoods, then targeted content searches. Where installed, use `vault --root ABSOLUTE_VAULT_ROOT find "concept" --limit 10`; otherwise use scoped filesystem search. Search synonyms, alternate notation in prose, and domain-specific terminology rather than relying solely on exact words. Do not upload the vault or install an embedding service.
